...
{
    public partial class FormModifierAccord : Form
    {
        private int PersonID = -1;
        private string Numero_Patient = null;
        private int Accord_ID = -1;
        Person person;
        Patient patient;
        Accord accord;
        Assure assure;
        ...
        public FormModifierAccord(int numero_patient)
        {
        ...
        private void btnEnregistrer_Click(object sender, EventArgs e)
        {
            ...
            {
                Accord.UpdateAccord_Produits(Accord_ID, oldReference1, tbxReference.Text, tbxDescription.Text, int.Parse(tbxQuantity.Text));
            }

            if (pnlProduit2.Visible && !string.IsNullOrWhiteSpace(tbxQuantity2.Text))
            {
                Accord.UpdateAccord_Produits(Accord_ID, oldReference2, tbxReference2.Text, tbxDescription2.Text, int.Parse(tbxQuantity2.Text));
            }

            if (pnlProduit3.Visible && !string.IsNullOrWhiteSpace(tbxQuantity3.Text))
            {
                Accord.UpdateAccord_Produits(Accord_ID, oldReference3, tbxReference3.Text, tbxDescription3.Text, int.Parse(tbxQuantity3.Text));
            }
