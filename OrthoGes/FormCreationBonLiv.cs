...
namespace OrthoGes
{
    public partial class FormCreationBonLiv : Form
    {
        ...
        private void btnEnregistrer_Click(object sender, EventArgs e)
        {
            ...
            if (patient.est_Assure == 1)
            {
                result = Bon_Livraison.CreateBonLiv(Numero_Patient, tbxReference.Text, DateTime.Parse(tbxDate.Text), Convert.ToInt32(tbxQuantity.Text), Convert.ToDecimal(tbxTVAMontant.Text), Convert.ToDecimal(tbxTotale.Text), tbxCentrePayeurPatient.Text, int.Parse(tbxTVA.Text),tbxPieceProduit.Text);
            }
            else
            {
                result = Bon_Livraison.CreateBonLiv(Numero_Patient, tbxReference.Text, DateTime.Parse(tbxDate.Text), Convert.ToInt32(tbxQuantity.Text), Convert.ToDecimal(tbxTVAMontant.Text), Convert.ToDecimal(tbxTotale.Text), tbxCentrePayeurAssure.Text, int.Parse(tbxTVA.Text),tbxPieceProduit.Text);
            }
            if (result)
            {
