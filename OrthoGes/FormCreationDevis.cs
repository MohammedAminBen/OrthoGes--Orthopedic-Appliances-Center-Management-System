...
namespace OrthoGes
{
    public partial class FormCreationDevis : Form
    {
        ...
        public FormCreationDevis(string numero_patient)
        {
            InitializeComponent();
        ...
        private void btnEnregistrer_Click(object sender, EventArgs e)
        {
            ...
            if(patient.est_Assure == 1)
            {
                result = Devis.CreateDevis(Numero_Patient, tbxReference.Text, DateTime.Parse(tbxDate.Text), Convert.ToInt32(tbxQuantity.Text), Convert.ToDecimal(tbxTVAMontant.Text), Convert.ToDecimal(tbxTotale.Text), tbxCentrePayeurPatient.Text,int.Parse(tbxTVA.Text));
            }
            else
            {
                result = Devis.CreateDevis(Numero_Patient, tbxReference.Text, DateTime.Parse(tbxDate.Text), Convert.ToInt32(tbxQuantity.Text), Convert.ToDecimal(tbxTVAMontant.Text), Convert.ToDecimal(tbxTotale.Text), tbxCentrePayeurAssure.Text, int.Parse(tbxTVA.Text));
            }
            if (result)
            {
